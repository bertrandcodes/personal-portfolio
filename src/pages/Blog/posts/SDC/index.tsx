...
const SDC = () => (
  <div className={styles.blogContainer}>
    ...
    <p className={styles.text}>
      For background, our team was given a functional legacy codebase to perform queries off of. The
      problem was that this existing database only housed 100 records. Each of us had to develop our
      own techniques to increase that figure to 10,000,000. Scary number isn’t it? As it turns out,
      ten million records is not much to brag about when it comes to large scale production sites
      (think{" "}
    ...
    <p className={styles.text}>
      ...
      the existing codebase UI copied a Nike product page, this meant generating shoe artifacts with
      fresh new information every time. With help from several helper functions, I created a random
      shoe generator that encapsulated data points such as shoe name, price, size- the kind of
      details that come to mind when you think about a Nike shoe.{" "}
    </p>
    <p className={styles.text}>
    ...
    <p className={styles.text}>
      ...
      reaches its capacity (or exceeds its highWaterMark property), the drain event fires to prevent
      more data from being written while simultaneously “flushing” the previously occupied memory.
      Doing so prevents high memory bottlenecks and crashes.
    </p>
    <img className={styles.image} src="/finished.jpg" alt="finished in 256.51 seconds" />
    <p className={styles.text}>
      And just like that, I had successfully occupied my JSON file with 10 million unique nike shoe
      objects in a little under 5 minutes.*
    </p>
    ...
    <p className={styles.text}>
      ...
      server had used MongoDB, but switching over to a relational (SQL) database such as PostgreSQL
      was also an option. To identify which better suited our needs, I had to weigh some pros and
      cons and benchmark the two.
    </p>
    <p className={styles.text}>
      Traditionally speaking, MongoDB and other document databases excel at working with formats
      such as JSON data. However, recently PostgreSQL has also begun offering enhanced JSON storage
      capabilities. Some{" "}
      <a
      ...
      will say that PostgreSQL offers transactional integrity and data optimization features at the
      cost of speed and performance. But at the same time, a{" "}
      <a
      ...
      fields whereas PostgreSQL required one to extract the whole document and rewrite it back after
      changes were made.
    </p>
    <p className={styles.text}>
    ...
    <p className={styles.text}>
      ...
      other. With a bit of performance tuning and optimization, I was able to achieve the following
      results:
    </p>
    <h4 className={styles.sectionHeader}>MongoDB:</h4>
    ...
    <p className={styles.text}>Sure enough, PostgreSQL saw an all around faster querying speed.</p>
    ...
    <p className={styles.text}>
      ...
      of around 1-2 ms, this was certainly a loss in performance. To do better, we needed to scale.
    </p>
    ...
    <p className={styles.text}>
      ...
      computers to the mix. Imagine a net made up of computers that grows horizontally to distribute
      the traffic load evenly. For learning purposes, we decided to see what strategies we could
      employ by horizontally scaling using AWS T2 micro instances.
    </p>
    <p className={styles.text}>
      Starting off with a single instance, I simulated 500 responses per second (RPS) over 1 minute
      using{" "}
    ...
    <p className={styles.text}>
      We briefly discussed the concept of horizontal scaling earlier, but how is it done? Turns out
      there were many ways of distributing the traffic over multiple server instances (also known as
      load balancing). After some research, I settled on using a popular, open-source software known
      as{" "}
    ...
    <p className={styles.text}>
      ...
      <a href="http://nginx.org/en/docs/http/load_balancing.html" target="_blank" rel="noreferrer">
        documentation
      </a>{" "}
    ...
    <p className={styles.text}>
      Where 1000 RPS had previously drawn an error rate of 34.8% and a latency of almost 500 ms, we
      now had a 0% error rate at a latency of 74 ms. Woot!
    </p>
    ...
    <p className={styles.text}>
      ...
      feat compared to a single running server. However, error rates started increasing at as little
      as 1500 RPS. For a practical user experience, we could not allow for errors like that. We had
      to do even better.
    </p>
    <p className={styles.text}>
    ...
    <h4 className={styles.sectionHeader}>2500 RPS at server 4:</h4>
    <img className={styles.image} src="/436ms.jpg" alt="436ms at 2500rps" />
    <h4 className={styles.sectionHeader}>2500 RPS at server 5:</h4>
    ...
    <p className={styles.text}>
      ...
      &quot;least-conn&quot;, where requests would be sent to the server most available to handle a
      new request. We tried that out:
    </p>
    <h4 className={styles.sectionHeader}>2800 RPS round-robin:</h4>
    <img className={styles.image} src="/roundrobin.jpg" alt="379ms at 2800rps round-robin" />
    <h4 className={styles.sectionHeader}>2800 RPS least-conn:</h4>
    ...
    <p className={styles.text}>
      ...
      . The idea of caching is that data gets stored in an accessible location, such that next time
      the data is called the server needs not reach all the way into the database to acquire it.
    </p>
    ...
    <p className={styles.text}>
      At the end of the day, my team and I learned a lot about what it takes for a production level
      site to handle traffic. We were exposed to the limitations of certain strategies and realized
      that tradeoffs were required when it came to optimizing a server. As it holds true for most
      cases in programming, there’s no one-size-fits-all method for server improvement. Overall, the
      main take away would be to keep benchmarking and testing new strategies to further optimize
      results. I hope you enjoyed following me on this journey and gained some knowledge or even
      inspiration from it. Remember, stay focused and always stay learning!
    </p>
  </div>
);
